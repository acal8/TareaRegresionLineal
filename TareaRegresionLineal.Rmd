...
Por lo que, pese a que el $R^2$ sea ligeramente menor (seguimos explicando algo más del 40% de la varianza de la variabel `crim`), podemos considerar que los fallos del anterior modelo se han solucionado con esta transformación.
...
Obtenemos un p-valor significativo al 0%, por lo que rechazamos la hipótesis nula y concluimos que el coeficiente cuadrático es significativo. Comprobamos también que el valor del AIC se reduce, con lo cual hemos mejorado con nuestro nuevo modelo.
...
Observamos que obtenemos un valor lógico tanto para la predicción puntual como para ambos intervalos.
...
De nuevo nuestro modelo predice unos valores razonables para la predicción puntual y el intervalo de confianza, a pesar de que el intervalo de predicción que obtenemos es demasiado amplio y poco representativo.

Por último, para los barrios con precio mediano de la vivienda de 100000 dólares, la tasa de criminalidad esperada es de `r format(int_conf[3,1], scientific = TRUE, digits = 4)`, con un intervalo de confianza de [`r format(int_conf[3,2], scientific = TRUE, digits = 4)`, `r format(int_conf[3,3], scientific = TRUE, digits = 4)`] y un intervalo de predicción de [`r format(int_pred[3,2], scientific = TRUE, digits = 4)`, `r format(int_pred[3,3], scientific = TRUE, digits = 4)`].

Notamos que obtenemos valores desproporcionados para el caso de los barrios con precio mediano de la vivienda de 100000 dólares. Esto se debe a que nuestro modelo ha sido ajustado para valores de la variable `medv` entre $7.2$ y $50.0$, por lo que tratar de hacer predicciones para valores tan alejados de este intervalo puede llevar a resultados fuera del rango lógico de la variable `crim`. El resultado lógico esperado, mirando la tendencia del conjunto de datos original, sería un valor cercano a cero,
...
```{r}
reg_crim <- regsubsets(crim ~ ., data = boston, nvmax = 10)
resumen <- summary(reg_crim)
print(resumen)
...
par(mfrow = c(1,3))
plot(1:10, resumen$adjr2, xlab = "# Variables", main = "Coef. Det. Ajustado", type="b")
abline(v = which.max(resumen$adjr2), col = 2)

plot(1:10, resumen$cp, xlab = "# Variables", main = "Cp de Mallows", type='b')
abline(v = which.min(resumen$cp), col = 2)

plot(1:10, resumen$bic, xlab = "# Variables", main = "BIC", type = "b")
abline(v = which.min(resumen$bic), col = 2)
```


```{r}
colnames(resumen$which)[resumen$which[5,]==T] #Vemos las variables necesarias

colnames(resumen$which)[resumen$which[8,]==T] #Vemos las variables necesarias
```

```{r}
best5 <- lm(crim ~ indus + chas + rm + ptratio + lstat, data = boston, na.action = na.exclude)
best8 <- lm(crim ~ indus + chas + rm + age + dis + ptratio + lstat + medv, data = boston, na.action = na.exclude)
