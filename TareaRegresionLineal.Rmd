...
En primer lugar, realizaremos una transformación logarítmica de los datos.
...
El p-valor que obtenemos ya no es menor que el nivel de significación $\alpha = 0.05$, por lo que no rechazamos la hipótesis nula y hay evidencia estadística para afirmar que el coeficiente de orden 3 no es significativo. Por tanto, nos quedamos con nuestro modelo parabólico.
...
Realizaremos las predicciones y los cálculos de los intervalos de confianza y predicción con el modelo parabólico entrenado sobre los datos con una transformación logarítmica. Debemos realizar la transformación inversa a los resultados (función exponecial) antes de darles una interpretación.

```{r}
predict_log <- predict.lm(lm_medv_parab, newdata = data.frame(medv = c(30, 10, 100)), se = T)
exp(predict_log$fit)
...
Para aquellos barrios con un precio mediano de la vivienda de 30000 dólares la tasa de criminalidad esperada es de `r`
