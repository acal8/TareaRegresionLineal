...
Para el test de normalidad de Shapiro-Wilk, obtenemos un p-valor de $0.02745$, por lo que, si consideráramos una significancia estadística del $1$% ($\alpha = 0.01$), podríamos rechazar la hipótesis nula y tener evidencia estadística a favor de la normalidad de los residuos.
...
Observando el resultado del p-valor cercano a $0$ en el test de normalidad de Shapiro-Wilk, rechazamos la hipótesis nula de normalidadcde los residuos, es decir, hay evidencia estadística para afirmar que los residuos no siguen una distribución normal.
...
lm_medv_parab <- lm(crim ~ medv + I(medv^2), data = boston)
summary(lm_medv_parab)

# Representación del ajuste
plot(boston$medv, boston$crim, col = 'black', main = "Diagrama de dispersión", xlab = "medv", ylab = "crim", ylim = c(-2, 25))
abline(coef = coef(lm_crim_medv), lwd = 2, col = 'red')
...
Para saber si el coeficiente cuadrático es significativo, utilizamos las funciones `anova` y `AIC`.

```{r}
anova(lm_medv_parab, lm_crim_medv)
print(paste("AIC del modelo lineal:", AIC(lm_crim_medv)))
print(paste("AIC del modelo parabólico:", AIC(lm_medv_parab)))
```

Obtenemos un p-valor significativo al 0%, por lo que rechazamos la hipótesis nula y concluimos que hay evidencia estadística a favor de la significatividad del coeficiente cuadrático. Comprobamos también que el valor del AIC se reduce.
...
```{r}
lm_medv_3 <- lm(crim ~ poly(medv, 3), data = boston)
anova(lm_medv_parab, lm_medv_3, test = 'F')
```

El p-valor que obtenemos es de nuevo pequeño, por lo que rechazamos la hipótesis nula y el coeficiente de orden 3 es significativo.

Para el coeficiente de orden cuatro:

```{r}
lm_medv_4 <- lm(crim ~ poly(medv, 4), data = boston)
anova(lm_medv_3, lm_medv_4, test = 'F')
```

El p-valor que obtenemos ya no es menor que el nivel de significación $\alpha = 0.05$, por lo que no rechazamos la hipótesis nula y hay evidencia estadística para afirmar que el coeficiente de orden 4 no es significativo.

Por tanto, para nuestro modelo de orden cúbico tenemos:

```{r}
# Planteamos una solución con un ajuste de orden 3 sobre la variable predictora medv
summary(lm_medv_3)

# Representación del ajuste
plot(boston$medv, boston$crim, col = 'black', main = "Diagrama de dispersión", xlab = "medv", ylab = "crim", ylim = c(-2, 25))
abline(coef = coef(lm_crim_medv), lwd = 2, col = 'red')
lines(sort(boston$medv), fitted(lm_medv_3)[order(boston$medv)], col = 'darkcyan', lwd = 3)
legend('topright', legend = c('Modelo lineal', 'Modelo de orden 3'), lwd = 3, col = c('red', 'darkcyan'))

# Diagnóstico de linealidad y homocedasticidad
residuos_3 <- residuals(lm_medv_3)
predichos_3 <- fitted.values(lm_medv_3)

plot(predichos_3, residuos_3, col='blue', main = 'Gráfica de residuos con modelo de orden 3', xlab = 'predichos', ylab = 'residuos')
abline(h = 0, lty = 2)

# Diagnóstico de normalidad de los residuos
qqnorm(residuos_3, col='blue')
qqline(residuos_3)
shapiro.test(residuos_3)
```

Observamos que con nuestro nuevo modelo obtenemos un valor $R^2$ ajustado de $0.6671$, es decir, explicamos aproximadamente $2/3$ de la varianza de la variable `crim`, frente a menos del $30%$ que explicábamos con el modelo de regresión lineal original.
